...
bucksWindow::bucksWindow(std::vector<Tilemap> *dung, std::vector<double> *runtime)
{
	m_DungPointer = dung;
...
void bucksWindow::run()
{
	...
	for (int i = 0; i < m_Buttons.size(); i++) {
		...
		else if (m_Buttons[i]->GetIsRooms())
			m_RoomCount = m_Buttons[i]->GetRoomCount();
				...
				if (event.mouseButton.button == sf::Mouse::Left) {
					for (int i = 0; i < m_Buttons.size(); i++) {
						if (m_Buttons[i]->GetButton().getGlobalBounds().contains(sf::Mouse::getPosition(*m_Window).x, sf::Mouse::getPosition(*m_Window).y)){
							if (m_Buttons[i]->GetIsMulti()) m_Buttons[i]->BoolSwitch(m_MultiRoom);
							else if (m_Buttons[i]->GetIsMap()) m_Buttons[i]->SetMapsize(m_MapSize);
							else if (m_Buttons[i]->GetIsRooms())m_Buttons[i]->SetRoomCount(m_RoomCount);
							else if (m_Buttons[i]->GetIsLaps()) m_Buttons[i]->SetLapCount(m_LapCount);
							else if (m_Buttons[i]->GetIsWinding()) m_Buttons[i]->SetWinding(m_WindingCount);
								...
								m_Tilemap.clear();
								Generated = true;
								m_Av = m_Index = m_HighIndex = m_LowIndex = 0;
								for (int k = 0; k < m_LapCount; k++) {
									m_Dungeons.push_back(DungeonGen(m_MapSize, m_MapSize, m_Window, 600, 600, m_RoomCount, m_MultiRoom, m_WindingCount));
									m_Runtime.push_back(m_Dungeons.back().runtime);
									m_Tilemap.push_back(m_Dungeons.back().GetTilemap());

									DungeonEvaluator eval = DungeonEvaluator(&m_Tilemap.back(), true, &m_Runtime.back());

									if (m_Dungeons.size() >= 1 && m_MapSize > 550) PopVectors();
									else if (m_Dungeons.size() > 1 && m_MapSize > 300) PopVectors();
	...
	*m_DungPointer = m_Tilemap;
	*m_RuntimePointer = m_Runtime;
}

bucksWindow::~bucksWindow()
{
	for (int i = 0; i < m_Buttons.size(); i++) {
		delete m_Buttons[i];
	}
...
void bucksWindow::CreateButtons()
{
	sf::Font myfont;
	...
	button* roomButton = new button(myfont, temp4, m_Window, 100, "Rooms", 3);
	m_Buttons.push_back(roomButton);

	sf::FloatRect temp5(1100 * x, 250 * y, 250 * x, 50 * y);
	button* lapsButton = new button(myfont, temp5, m_Window, 100, "Laps", 4);
	...
	button* MultiConnect = new button(myfont, temp10, m_Window, 0, 9);
	m_Buttons.push_back(MultiConnect);

	sf::FloatRect temp11(1100 * x, 325 * y, 250 * x, 50 * y);
	button* windingButton = new button(myfont, temp11, m_Window, 100, "Winding", 10);
	m_Buttons.push_back(windingButton);

}

void bucksWindow::PopVectors()
{
	m_Dungeons.pop_back();
	//m_Runtime.pop_back();
	m_Tilemap.pop_back();
...
void bucksWindow::ClearTextfiles()
{
	std::string runtimeName = "Bucks_" + std::to_string(m_MapSize) + "_Runtime.txt";
	std::string densityName = "Bucks_" + std::to_string(m_MapSize) + "_Density.txt";
	std::string relativesizeName = "Bucks_" + std::to_string(m_MapSize) + "_RelativeSize.txt";
